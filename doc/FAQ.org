...
- [[#linux][Linux]]
  ...
  - [[#speed-up-spacemacs-with-package-quickstart][Speed up Spacemacs with package-quickstart]]
- [[#windows][Windows]]
...
** Speed up Spacemacs with package-quickstart
The ~package-quickstart~ will collect all the ~*-autoloads.el~ into one file
~package-quickstart-file~ to speed up the packages loading procedure.

After Spacemacs startup, manually execute the function
~package-quickstart-refreshe~ to generate the package quickstart file,
then set the dotfile variable ~dotspacemacs-enable-package-quickstart~
to ~t~.  On the next startup, the Spacemacs will set up packages by
loading the ~package-quickstart-file~ rather than calling
~package-initialize~ (which will scan ~package-user-dir~ and activate
packages one by one).

*NOTE*: Consider enabling the ~load-hints~ feature first, otherwise
the ~package-quickstart-file~ adds all the package directories to
~load-path~, and may lead to Spacemacs starting up slower than without
~package-quickstart~ feature enabled.
